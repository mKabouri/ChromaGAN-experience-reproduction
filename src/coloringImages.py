...
def coloringSamples(colorization, nbSamples, dirToSave, pathToX_test, pathToy_test, weightsFileName):
    ...
    colorImages = np.array([colorization.predict(x.reshape(1, config.ImageSize, config.ImageSize, 1)) for x in batchX])
    for i in range(nbSamples):
        if config.dataset == 'Div2K':
            print(y_test[i+n])
            res = np.concatenate((np.concatenate((denormalize(y_test[i+n]), denormalize(np.stack((batchX[i].reshape(config.ImageSize, config.ImageSize),)*3, axis=-1)))), denormalize(colorImages[i].reshape(config.ImageSize, config.ImageSize, 3))))
        else:   
            if config.ImagesFormat == 'Lab':
...
if __name__ == '__main__':
    ...
    if config.dataset == 'Div2K':
        numChannels = 3
        weightsFileName = 'Div2KWithSkipRGBWeights.h5'
        pathToX_test = 'X_train_Div2K.npy'
        pathToy_test = 'y_train_Div2K.npy'
    ...
    colorizationModel.build((1, config.ImageSize, config.ImageSize, 1))
